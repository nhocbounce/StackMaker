...
public class PlayerBlock : Block
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.CompareTag(Constants.WIN))
        {
            ...
            {
                Destroy(standingBlock.Pop());
                Invoke(nameof(WinGame), Constants.DELAYSHOWINGUI);
            }
        }
    }
    ...
    Vector3 ModelPos()
    {
        Vector3 v3 = transform.position + new Vector3(0, standingBlock.Count * Constants.STACKINGBLOCKTHICKNESS - Constants.MODELHEIGHTOFFSET, 0);
        return v3;
    }
    Vector3 StackPos()
    {
        Vector3 v3 = transform.position + new Vector3(0, standingBlock.Count * Constants.STACKINGBLOCKTHICKNESS, 0);
        return v3;
    }
}
